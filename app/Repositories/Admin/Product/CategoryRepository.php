...
class CategoryRepository implements CategoryInterface
{
    ...
    public function all()
    {
        return Category::leftJoin('category_languages', 'category_languages.category_id', '=', 'categories.id')
            ->select('categories.*', 'category_languages.id as category_lang_id', 'category_languages.title', 'category_languages.lang',
                'category_languages.meta_title', 'category_languages.meta_description', 'category_languages.category_filter', 'category_languages.number', 'category_languages.soc_fb', 'category_languages.soc_yt', 'category_languages.soc_in');
    }
    ...
    public function search($key, $paginate, $user=null){
        return Category::with('childCategories','categoryLanguage')
            ->whereHas('categoryLanguage', function ($query) use ($key) {
...
//            $category->is_digital        = $request->is_digital;
    ...
    public function homePageCategory()
    {
        ...
        return Category::with('childCategories')->where('parent_id',null)->where('status',1)->orderBy('ordering')->take($take)->get();
    }
    ...
    public function categoryByIds($top_category,$take= null)
    {
        if ($take)
        {
            $categories = Category::whereIn('id',$top_category)->select('id','logo','slug','icon','parent_id','banner')->where('status',1)->get();
        }
        else{
            $categories = Category::whereIn('id',$top_category)->select('id','logo','slug','icon','parent_id','banner')->where('status',1)->get();
        }
