...
//            JWTAuth::invalidate(JWTAuth::getToken());
    ...
    public function shopDetails(Request $request,SellerInterface $sellers): \Illuminate\Http\JsonResponse
    {
        ...
        $data = [
                ...
                'tax_paper'             => $user->sellerProfile->tax_paper !=[] && is_file_exists(@$user->sellerProfile->tax_paper['image_72x72']) ?
                    getFileLink('72x72',$user->sellerProfile->tax_paper) : static_asset('images/default/default-image-400x235.png')
        ];
        try{
    ...
    public function homeScreen(Request $request,ProductInterface $product,OrderInterface $order): \Illuminate\Http\JsonResponse
    {
        try{
            ...
            $shop                   = $user->sellerProfile;
            $products               = $product->productCount([$user->id]);
            ...
            $data = [
              ...
              'logo'                    =>   static_asset($shop->logo['image_72x72']),
    ...
    public function dataAnalysis(Request $request,ProductInterface $product,OrderInterface $order): \Illuminate\Http\JsonResponse
    {
        try{
            ...
            $data = [
                'earning'                   =>   $order->all()->where('delivery_status', 'delivered')
                    ->where('seller_id',$user->id)->when(arrayCheck('start_time',$request) && arrayCheck('end_time',$request),function ($query) use ($request){
                        $query->whereBetween('date',[$request['start_time'],$request['end_time']]);
                    })->sum('total_payable'),
                'order_place'               =>  $order->all()->where('delivery_status', 'delivered')
                    ->where('seller_id',$user->id)->when(arrayCheck('start_time',$request) && arrayCheck('end_time',$request),function ($query) use ($request){
                        $query->whereBetween('data',[$request['start_time'],$request['end_time']]);
                    })->count(),
